import { IBatteryProduct, IServiceArea, ITestimonial, IFAQItem, IHowItWorksStep } from '../types';

export const mockBatteryProducts: IBatteryProduct[] = [
  {
    id: 'gs-ns60ls',
    name: 'GS ASTRA Maintenance Free NS60LS / 46B24LS',
    category: 'GS ASTRA',
    capacityAh: 45,
    voltage: 12,
    price: 895000,
    originalPrice: 1050000,
    image: '/images/batteries/gs-ns60ls.webp',
    compatibleCars: ['Honda CR-V', 'Honda Civic', 'Toyota Yaris', 'Toyota Vios', 'Mazda 2', 'Nissan Grand Livina'],
    vehicleType: 'Sedan/Hatchback',
    warrantyMonths: 24,
    isBestSeller: true,
    technology: 'Maintenance Free (Bebas Perawatan)',
  },
  {
    id: 'gs-ns40zl',
    name: 'GS ASTRA MF NS40ZL / 36B20L',
    category: 'GS ASTRA',
    capacityAh: 35,
    voltage: 12,
    price: 745000,
    originalPrice: 880000,
    image: '/images/batteries/gs-ns40zl.webp',
    compatibleCars: ['Toyota Avanza', 'Daihatsu Xenia', 'Toyota Calya', 'Daihatsu Sigra', 'Honda Brio', 'Suzuki Ertiga'],
    vehicleType: 'City Car',
    warrantyMonths: 24,
    isBestSeller: true,
    technology: 'Maintenance Free (Bebas Perawatan)',
  },
  {
    id: 'amaron-hilife-55d23l',
    name: 'AMARON Hi-Life 55D23L',
    category: 'AMARON',
    capacityAh: 60,
    voltage: 12,
    price: 1250000,
    originalPrice: 1450000,
    image: '/images/batteries/amaron-55d23l.webp',
    compatibleCars: ['Toyota Innova Bensin', 'Nissan X-Trail', 'Subaru XV', 'Mazda CX-5', 'Mitsubishi Outlander'],
    vehicleType: 'SUV/MPV',
    warrantyMonths: 24,
    isBestSeller: true,
    technology: 'Silven X Alloy Long Life',
  },
  {
    id: 'gs-n70z',
    name: 'GS ASTRA Heavy Duty N70Z / 95D31R',
    category: 'GS ASTRA',
    capacityAh: 75,
    voltage: 12,
    price: 1420000,
    originalPrice: 1650000,
    image: '/images/batteries/gs-n70z.webp',
    compatibleCars: ['Toyota Fortuner Diesel', 'Mitsubishi Pajero Sport', 'Isuzu Panther', 'Toyota Hilux', 'Ford Ranger'],
    vehicleType: 'Diesel/Commercial',
    warrantyMonths: 18,
    isBestSeller: false,
    technology: 'Heavy Duty Hybrid',
  },
  {
    id: 'amaron-din-74',
    name: 'AMARON Pro DIN 74Ah',
    category: 'AMARON',
    capacityAh: 74,
    voltage: 12,
    price: 1850000,
    originalPrice: 2100000,
    image: '/images/batteries/amaron-din74.webp',
    compatibleCars: ['BMW Seri 3/5', 'Mercedes-Benz C-Class', 'Audi A4', 'Volkswagen Golf', 'Peugeot 3008'],
    vehicleType: 'Sedan/Hatchback',
    warrantyMonths: 24,
    isBestSeller: false,
    technology: 'European DIN Spec Silver',
  },
  {
    id: 'varta-efb-n70',
    name: 'VARTA EFB Start-Stop N70',
    category: 'VARTA',
    capacityAh: 70,
    voltage: 12,
    price: 2150000,
    originalPrice: 2400000,
    image: '/images/batteries/varta-efb.webp',
    compatibleCars: ['Mazda CX-30 / CX-5 I-Stop', 'Hyundai Santa Fe', 'Kia Sorento', 'Toyota Camry Hybrid'],
    vehicleType: 'SUV/MPV',
    warrantyMonths: 24,
    isBestSeller: true,
    technology: 'Enhanced Flooded Battery (EFB)',
  },
];

export const mockServiceAreas: IServiceArea[] = [
  // JABODETABEK
  { id: 'jkt-sel', city: 'Jakarta Selatan', province: 'DKI Jakarta', region: 'JABODETABEK', status: 'ACTIVE_24H', estimatedTimeMinutes: 8, techniciansCount: 28 },
  { id: 'jkt-bar', city: 'Jakarta Barat', province: 'DKI Jakarta', region: 'JABODETABEK', status: 'ACTIVE_24H', estimatedTimeMinutes: 10, techniciansCount: 24 },
  { id: 'jkt-pus', city: 'Jakarta Pusat', province: 'DKI Jakarta', region: 'JABODETABEK', status: 'ACTIVE_24H', estimatedTimeMinutes: 9, techniciansCount: 20 },
  { id: 'jkt-utr', city: 'Jakarta Utara', province: 'DKI Jakarta', region: 'JABODETABEK', status: 'ACTIVE_24H', estimatedTimeMinutes: 12, techniciansCount: 18 },
  { id: 'jkt-tim', city: 'Jakarta Timur', province: 'DKI Jakarta', region: 'JABODETABEK', status: 'ACTIVE_24H', estimatedTimeMinutes: 10, techniciansCount: 26 },
  { id: 'tng-kota', city: 'Tangerang Kota & BSD', province: 'Banten', region: 'JABODETABEK', status: 'ACTIVE_24H', estimatedTimeMinutes: 10, techniciansCount: 22 },
  { id: 'tng-sel', city: 'Tangerang Selatan & Bintaro', province: 'Banten', region: 'JABODETABEK', status: 'ACTIVE_24H', estimatedTimeMinutes: 8, techniciansCount: 25 },
  { id: 'bks', city: 'Bekasi Kota & Cikarang', province: 'Jawa Barat', region: 'JABODETABEK', status: 'ACTIVE_24H', estimatedTimeMinutes: 12, techniciansCount: 24 },
  { id: 'bgr', city: 'Bogor & Sentul', province: 'Jawa Barat', region: 'JABODETABEK', status: 'ACTIVE_24H', estimatedTimeMinutes: 15, techniciansCount: 16 },
  { id: 'dpk', city: 'Depok & Margonda', province: 'Jawa Barat', region: 'JABODETABEK', status: 'ACTIVE_24H', estimatedTimeMinutes: 10, techniciansCount: 18 },

  // JAWA & BALI
  { id: 'bdg', city: 'Bandung Kota & Cimahi', province: 'Jawa Barat', region: 'JAWA_BALI', status: 'ACTIVE_24H', estimatedTimeMinutes: 10, techniciansCount: 30 },
  { id: 'smg', city: 'Semarang & Ungaran', province: 'Jawa Tengah', region: 'JAWA_BALI', status: 'ACTIVE_24H', estimatedTimeMinutes: 12, techniciansCount: 18 },
  { id: 'solo', city: 'Solo & Sukoharjo', province: 'Jawa Tengah', region: 'JAWA_BALI', status: 'ACTIVE_24H', estimatedTimeMinutes: 10, techniciansCount: 15 },
  { id: 'jog', city: 'Yogyakarta & Sleman', province: 'DI Yogyakarta', region: 'JAWA_BALI', status: 'ACTIVE_24H', estimatedTimeMinutes: 10, techniciansCount: 20 },
  { id: 'sby', city: 'Surabaya & Sidoarjo', province: 'Jawa Timur', region: 'JAWA_BALI', status: 'ACTIVE_24H', estimatedTimeMinutes: 9, techniciansCount: 35 },
  { id: 'mlg', city: 'Malang Kota', province: 'Jawa Timur', region: 'JAWA_BALI', status: 'ACTIVE_24H', estimatedTimeMinutes: 12, techniciansCount: 16 },
  { id: 'dps', city: 'Denpasar & Badung (Bali)', province: 'Bali', region: 'JAWA_BALI', status: 'ACTIVE_24H', estimatedTimeMinutes: 10, techniciansCount: 20 },

  // LUAR JAWA
  { id: 'mdn', city: 'Medan Kota', province: 'Sumatera Utara', region: 'LUAR_JAWA', status: 'ACTIVE_24H', estimatedTimeMinutes: 12, techniciansCount: 22 },
  { id: 'pbm', city: 'Palembang Kota', province: 'Sumatera Selatan', region: 'LUAR_JAWA', status: 'ACTIVE_24H', estimatedTimeMinutes: 15, techniciansCount: 16 },
  { id: 'pkn', city: 'Pekanbaru', province: 'Riau', region: 'LUAR_JAWA', status: 'ACTIVE_24H', estimatedTimeMinutes: 12, techniciansCount: 14 },
  { id: 'mks', city: 'Makassar Kota', province: 'Sulawesi Selatan', region: 'LUAR_JAWA', status: 'ACTIVE_24H', estimatedTimeMinutes: 12, techniciansCount: 18 },
  { id: 'bpn', city: 'Balikpapan & Samarinda', province: 'Kalimantan Timur', region: 'LUAR_JAWA', status: 'ACTIVE_24H', estimatedTimeMinutes: 15, techniciansCount: 15 },
];

export const mockTestimonials: ITestimonial[] = [
  {
    id: 't-1',
    customerName: 'Bapak Hendra Wijaya',
    carModel: 'Toyota Fortuner 2.4 Diesel',
    city: 'Jakarta Selatan',
    rating: 5,
    reviewText: 'Mobil mogok pas jam 2 malam di tol Dalam Kota. Panggil ShopDrive via WA, cuma 12 menit teknisi Mas Budi sampai bawa aki GS Astra ori. Tukar tambah juga dihargai tinggi. Penyelamat banget!',
    date: 'Kemarin, 02.15 WIB',
    verifiedPurchase: true,
  },
  {
    id: 't-2',
    customerName: 'Ibu Dr. Ratna Permata',
    carModel: 'Honda HR-V Prestige',
    city: 'Tangerang Selatan',
    rating: 5,
    reviewText: 'Mau berangkat rapat jam 7 pagi aki tiba-tiba mati total. Untung dapet kontak ShopDrive. Teknisi datang cepat, sopan, langsung riset kelistrikan dan pasang aki Amaron. Garansi 2 tahun terjamin!',
    date: '3 Hari Yang Lalu',
    verifiedPurchase: true,
  },
  {
    id: 't-3',
    customerName: 'Mas Rian Febrian',
    carModel: 'Mazda CX-5 GT',
    city: 'Bandung',
    rating: 5,
    reviewText: 'Teknisi paham betul spesifikasi aki i-Stop Mazda. Pakai VARTA EFB khusus. Pemasangan rapi pakai memory saver jadi settingan audio & jam gak hilang. Rekomendasi 100%!',
    date: '1 Minggu Yang Lalu',
    verifiedPurchase: true,
  },
];

export const mockFAQs: IFAQItem[] = [
  {
    id: 'faq-1',
    question: 'Berapa lama estimasi teknisi ShopDrive sampai ke lokasi saya?',
    answer: 'Rata-rata kedatangan teknisi kami adalah di bawah 10 hingga 15 menit untuk wilayah perkotaan utama (Jabodetabek, Bandung, Surabaya, Medan, Bali). Kami menempatkan teknisi siaga di titik-titik strategis.',
    category: 'PEMESANAN',
  },
  {
    id: 'faq-2',
    question: 'Apakah ada biaya tambahan untuk ongkos kirim dan pasang?',
    answer: 'TIDAK ADA BIAYA TERSEBUNYI! Layanan Pengiriman dan Pemasangan aki oleh teknisi bersertifikasi 100% GRATIS (sudah termasuk dalam harga aki yang tertera).',
    category: 'PEMBAYARAN',
  },
  {
    id: 'faq-3',
    question: 'Apakah aki yang dijual 100% Baru dan Asli bergaransi?',
    answer: 'Ya, seluruh produk aki di ShopDrive 100% Baru dari pabrikan resmi (GS Astra, Amaron, Varta) dengan barcode seri resmi dan garansi resmi hingga 24 bulan.',
    category: 'GARANSI',
  },
  {
    id: 'faq-4',
    question: 'Bagaimana jika saya tidak tahu tipe/ukuran aki yang cocok untuk mobil saya?',
    answer: 'Tim customer service dan teknisi kami siap membantu melakukan pengecekan gratis berdasarkan merk, tahun, dan tipe mobil Anda sebelum aki dikirim.',
    category: 'TEKNIS',
  },
  {
    id: 'faq-5',
    question: 'Apakah ShopDrive menerima Tukar Tambah (Trade-In) aki bekas?',
    answer: 'Tentu! Kami menerima tukar tambah aki lama Anda dengan harga potongan yang sangat kompetitif langsung mengurangi total pembayaran di tempat.',
    category: 'PEMBAYARAN',
  },
];

export const mockHowItWorksSteps: IHowItWorksStep[] = [
  {
    stepNumber: 1,
    title: 'Hubungi Hotline / WA 24 Jam',
    description: 'Klik tombol SOS atau chat WA. Informasikan lokasi mogok dan merk/tipe mobil Anda.',
    iconName: 'PhoneCall',
  },
  {
    stepNumber: 2,
    title: 'Teknisi Meluncur <10 Menit',
    description: 'Sistem dispatch kami mengutus teknisi bersertifikat terdekat langsung menuju posisi Anda.',
    iconName: 'Zap',
  },
  {
    stepNumber: 3,
    title: 'Tes Kelistrikan & Pasang',
    description: 'Teknisi menguji alternator & starter gratis sebelum memasang aki baru dengan memory saver.',
    iconName: 'Wrench',
  },
  {
    stepNumber: 4,
    title: 'Garansi Resmi & Pembayaran',
    description: 'Cek garansi resmi digital 24 bulan dan bayar mudah di tempat (Cash/QRIS/Transfer).',
    iconName: 'ShieldCheck',
  },
];
